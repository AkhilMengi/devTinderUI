            ...
            {showModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm transition-opacity">
                    <div className="relative w-[90%] max-w-md rounded-3xl border border-white/20 bg-white/10 p-8 shadow-2xl backdrop-blur-xl text-white animate-fade-in">
                        ...
                        <p className="text-center text-sm text-gray-200 mb-4 capitalize">{user.gender || "Not specified"}</p>

                        <div className="space-y-2 text-sm text-gray-100">
                            <p><span className="font-medium">Status:</span> <span className="capitalize">{request.status}</span></p>
                            <p><span className="font-medium">Request ID:</span> {request._id}{request.connectionId}</p>
                            {showActions && (
